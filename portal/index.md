...
> ※以下は現時点では **検討段階のテーマメモ** であり、  
> 実装・詳細設計・章立ては未確定とする。

- 🧩 **AITL Controller A-Type**
  - V–I表記に基づく制御ASIC（SKY130）
  - 実時間PID制御を中心とした最小構成
...
- ⏳ **長期動作劣化（〜1000 day）影響検討**
  - デバイス劣化を想定した制御性能変化
  - 再同定・再設計が必要となる条件整理

- 🧠 **AITL Controller B-Type**
  - 劣化・状態変化を考慮した制御構成
  - FSMによる状態監督と再設計前提アーキテクチャ

- 🧬 **OpenLane + RISC-V 統合構成**
  - 制御アクセラレータ＋非実時間制御層
  - 設計フロー・I/F構成の検討
...
> 🔧 本テーマ群は、実装着手時点で内容が大きく拡張・再構成される可能性がある。
...
※ 本デモでは、外乱が PID + FSM の範囲で回復可能なため、
NN / RL および LLM は起動せず、
「起動しないこと自体」が責務分離の例として示される。
