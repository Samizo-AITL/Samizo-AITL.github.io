...
> 本ポータルは、  
> **物理モデル・制御理論・AIを分離統合し、  
> 実装と説明責任を両立できる設計体系を示す技術アーカイブ**です。  
>  
> リアルタイム制御は **PID**、  
> 状態と判断の明確化は **FSM**、  
> **AI（LLM）は設計・再設計支援のみに限定**しています。  
>  
> AIを制御に直接用いないことで、  
> **安全性・再現性・評価可能性を前提とした工学設計**を対象としています。
...
- **PID（実時間制御核）**  
  実時間制御を担当。  
  通常状態では **PID のみで安定追従**を行う。

- **FSM（監督・判断層）**  
  状態監督を担当。誤差や劣化を検出し、  
  *monitoring → disturbance → recovery → stable*  
  の状態遷移を管理する。  
  **実時間適応（NN / RL）や再設計（LLM）を許可するかどうかの最終判断権**を持つ。

- **NN / RL（実時間適応補助層）**  
  FSM によって許可された場合のみ介入し、  
  実時間で **限定的（bounded）な補助項**として振る舞う。  
  **PID 制御を置き換えない。**
...
- **LLM（非実時間・設計支援層）**  
  ...
  FSM に呼ばれた場合のみ介入し、  
  ログ解析・信頼性評価を通じて  
  **設計レベル（例：PID ゲインや制御方針）の再検討**を支援する。  
  **実時間制御には一切関与しない。**
...
1. **通常**：PID 追従 ／ FSM 監視 ／ NN・RL・LLM 待機  
2. **外乱**：誤差増大 → FSM が劣化を検出  
3. **適応（条件付き）**：FSM が許可した場合のみ NN / RL が限定的に介入  
4. **再設計（必要時）**：FSM が LLM を呼び出し、設計レベルの再検討を実施  
5. **回復**：PID 再追従 → FSM stable → NN / RL・LLM idle
